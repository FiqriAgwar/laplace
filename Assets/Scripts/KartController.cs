...
public class KartController : NetworkBehaviour
{
    ...
    [Header("Boost")]
    [SerializeField] private float maxBoostTime = 3f;
    [SerializeField] private float maxBoostSpeed = 50;
    private float boostTime = 0;
    private bool boostMode = false;
    private float boostDriveForce = 0;
    ...
    [Command]
    private void CmdUpdateBoostTime(){
        //Validation
    ...
    [ClientRpc]
    private void RpcUpdateBoostTime(){
        UpdateBoostTime();
    }
    ...
    public void Boost(float boostMultiplier){
        boostMode = true;
        boostDriveForce = driveForce * boostMultiplier;
        boostTime = maxBoostTime;
    }

    [Command]
    private void CmdDriveForward(){
        //Validation
    ...
    [ClientRpc]
    private void RpcDriveForward(){
        DriveForward();
    }

    private void DriveForward () {
        // If press forward & not over speed limit
        if (Input.GetAxisRaw("Vertical") > 0 && !boostMode && (Mathf.Pow(maxSpeed, 2) > rb.velocity.sqrMagnitude))
        {
            rb.AddForce(driveForce * modelRotator.forward);
        }
        else if (Input.GetAxisRaw("Vertical") > 0 && boostMode && (Mathf.Pow(maxBoostSpeed, 2) > rb.velocity.sqrMagnitude))
        {
            rb.AddForce(boostDriveForce * modelRotator.forward);
        }
    }

    [Command]
    private void CmdDriveBackward(){
        //Validation
    ...
    [ClientRpc]
    private void RpcDrift(){
        Drift();
    }

    private void Drift ()
    {
        bool driftCheck = Input.GetButton("Drift") && forwardSpeed > 0 && Input.GetAxisRaw("Vertical") > 0;
